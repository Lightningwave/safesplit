...
func (s *DistributedStorageService) RetrieveShards(fileID uint, totalShards int) ([][]byte, error) {
	...
	retrievedCount := 0
	dataShards := totalShards - 2 // Since we know it's using 4+2 configuration
	...
	for shardIndex := 0; shardIndex < totalShards; shardIndex++ {
		...
		if err != nil {
			...
			// If shard doesn't exist, leave it as nil and continue
			continue
		}
	...
	// Only require dataShards number of valid shards
	if retrievedCount < dataShards {
		return nil, fmt.Errorf("insufficient shards for reconstruction: found %d, need %d",
