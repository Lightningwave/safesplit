...
const SharedFileAccess = () => {
    ...
    const fetchFileInfo = async () => {
        try {
            const endpoint = `http://localhost:8080/api/${isPremiumShare ? 'premium/shares' : 'files/share'}/${shareId}`;
    ...
    const handleVerification = async (e) => {
        ...
        try {
            const endpoint = `http://localhost:8080/api/${isPremiumShare ? 'premium/shares' : 'files/share'}/${shareId}/verify`;
            const response = await fetch(endpoint, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
