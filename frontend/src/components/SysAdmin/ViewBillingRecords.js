...
const ViewBillingRecords = () => {
    ...
    const fetchBillingRecords = async (page, status) => {
        try {
            ...
            let url = `/api/system/billing/records?page=${page}&page_size=10`;
    ...
    const fetchStats = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch('/api/system/billing/stats', {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
    ...
    const fetchExpiringSubscriptions = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch('/api/system/billing/expiring', {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
